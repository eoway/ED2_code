...
individual_vars_mnth = ['year','DBH','HITE','CROWN_RADIUS','AGB_CO','MMEAN_LAI_CO','PFT','PATCH_AGE','X_COOR','Y_COOR'] # EO 
individual_vars_yr   = ['year','DBH','HITE','CROWN_RADIUS','AGB_CO','LAI_CO','PFT','PATCH_AGE','X_COOR','Y_COOR'] # EO  
...
dbh_size_list = ('D',[0,10,20,40,70,100])
hite_size_list = ('H',[0,10,20,40,60,80,100]) # EO
#hite_size_list = ('H',[0,10,30,40,50,60]) # EO
dbh_size_list_fine = ('D',np.arange(0.,200.+5.,10.))
hite_size_list_fine = ('H',np.arange(0.,100.+1.,6.)) # EO
#hite_size_list_fine = ('H',np.arange(0.,60.+1.,5.)) # EO
##################################################
...
def extract_individual_for_plot_annual(
     h5in           : 'handle for .h5 file'
    ,output_dict    : 'dictionary to store output data'
    ,pft_list       : 'List of PFTs to include'
    ...
    '''
        ...
        Divide the area into 16 25by25m patches and assign the patches according to patch area
        Only include cohorts >= 1.5 m
        NPLANT < 1 would be scaled up to 1
        include
        DBH, HITE, CROWN_RADIUS, AGB_CO, LAI_CO, NPLANT, PFT, X_data, Y_data (in meters) #EO
    '''
    ...
    AGB_CO    = np.array(h5in['AGB_CO']) # units? EO
    LAI_CO    = np.array(h5in['LAI_CO']) # m2/m2 EO   
    ...
    # get the number of plants to plot for each cohort
    plot_nplant_nums = np.maximum(1.,np.around(NPLANT * plot_size))
    plot_nplant_nums[HITE < 1.0] = 0. # do not plot cohorts smaller than 1.0m
    ...
    # generate a random state class implementation
    ...
    for ipa in np.arange(len(PACO_ID)):
        ...
        for i, ico in enumerate(cohort_ids):
            ...
            for ind_id in np.arange(plot_nums):
                ...
                output_dict['AGB_CO'].append(AGB_CO[ico]) ## EO 
                output_dict['LAI_CO'].append(LAI_CO[ico]) ## EO   added
                output_dict['PATCH_AGE'].append(AGE[ipa])
...
def extract_demo_rates_annual(
    ...
    ,pft_list : 'PFT to include'
    ,time_idx : 'index of the array'
    ,size_list : 'size_list' = dbh_size_list
    ):
    '''
       This function will use the BASAL_AREA_SI, BASAL_AREA_GROWTH, BASAL_AREA_MORT
    '''
    ...
    # create a year list and a month list to loop over
...
def extract_individual_for_plot_monthly(
     h5in           : 'handle for .h5 file'
    ,output_dict    : 'dictionary to store output data'
    ,pft_list       : 'List of PFTs to include'
    ,year           : 'Year of the plot information'
    ...
    CROWN_RADIUS = (CROWN_RADIUS / NPLANT / np.pi) ** (0.5)  # m
    AGB_CO    = np.array(h5in['AGB_CO']) # units? EO
    MMEAN_LAI_CO    = np.array(h5in['MMEAN_LAI_CO']) # m2/m2 EO   
    ...
    for ipa in np.arange(len(PACO_ID)):
        ...
        for i, ico in enumerate(cohort_ids):
            ...
            for ind_id in np.arange(plot_nums):
                ...
                output_dict['CROWN_RADIUS'].append(CROWN_RADIUS[ico])
                output_dict['AGB_CO'].append(AGB_CO[ico]) ## EO 
                output_dict['MMEAN_LAI_CO'].append(MMEAN_LAI_CO[ico]) ## EO    
                output_dict['PATCH_AGE'].append(AGE[ipa])
                output_dict['PFT'].append(PFT[ico])
                output_dict['X_COOR'].append(x_data)
                output_dict['Y_COOR'].append(y_data)
...
def extract_demo_rates_monthly(
    ...
    ,dbh_min        : 'the smallest tree to census [cm]' = 0.1
    ...
    '''
       ...
       (2) It will calculate the growth rates and mortality during the census rate for each initial
       cohort
    '''
    # Necessary stamps, time, cohort_id at the last 'census' in the model
    # cohorts that did not survive to the last time point will be given an id
