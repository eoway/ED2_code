...
from .extract_utils import individual_vars_mnth
from .extract_utils import individual_vars_yr
...
#    ,voi_annual  : 'Variable of interest to extract' = ['AGB','LAI','BA','NPLANT'] 
...
#        output_demography_dict[var] = np.zeros((len(size_year_array),))
    ...
    for var in voi:  # EO was just voi not 'voi_annual'
        for pft in pft_list:
    ...
    for iyear, year in enumerate(size_year_array):
        ...
        exut.extract_individual_for_plot_annual(h5in,output_individual_dict,
                                        pft_list,year)
...
#    del csv_df
    ...
    if not os.path.isfile(data_fn):
        print('{:s} does not exist!'.format(data_fn))
    ...
    csv_df.to_csv(csv_fn,index=False,mode='w',header=True)#,float_format='%g') # change float format? #EO
    del csv_df
...
def extract_monthly(
    ...
    ,voi            : 'variable of interests (ecosystem average)' = ['AGB','MMEAN_LAI','BA','NPLANT']  
    ,pft_list       : 'List of PFTs to include for pft VOIs' = []
 ):
    '''
    ...
    output_flux_vars = ['MMEAN_GPP_PY','MMEAN_NPP_PY','MMEAN_NEP_PY','MMEAN_CARBON_AC_PY',
    ...
    # we work backward since we only track the survived cohorts....
    ...
    for icensus, census_end in enumerate(census_years):
        ...
        # get individual file
        output_individual_dict ={}
        for var in individual_vars_mnth:
            output_individual_dict[var] = []
        ...
        # extract individual vars
        exut.extract_individual_for_plot_monthly(h5in,output_individual_dict,
                                        pft_list,census_end)
    ...
    for itime, year, month in zip(np.arange(len(year_list)),year_list,month_list):
        ...
        if month == last_month_of_year and itime > 0:
            ...
            for ico, dbh in enumerate(DBH):
                # find whether we can find the same cohort in the last year
                cohort_mask = (
