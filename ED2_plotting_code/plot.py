    ...
    for isp in np.arange(split_num):
        ...
        for isize, size_edge in enumerate(dbh_size_list[1]):
            if size_edge == dbh_size_list[1][-1]:
                # last one
                size_names.append('DBH > {:d}'.format(size_edge))
            else:
                size_names.append('DBH: {:d}-{:d}'.format(size_edge,
        ...
        for isize, size_edge in enumerate(hite_size_list[1]):
            if size_edge == hite_size_list[1][-1]:
                # last one
                size_names.append('Hgt > {:4.2f}'.format(size_edge))
            else:
                size_names.append('Hgt: {:4.2f}-{:4.2f}'.format(
...
#    if len(patch_age) > 2: #4: #EO
#        # more than 2 / 4 patches
#        patch_age_split = np.array_split(patch_age,2) #4) #EO
#        patch_age_to_plot = []
#        for i in np.arange(2): #4): #EO
...
#
#    row_num = np.maximum(2,patch_num) #4,patch_num) #EO
#
...
def plot_monthly(
    ...
    voi_labels = ['Tair (deg C)','P (mm month$^{-1}$)','Rshort (W m$^{-2}$)','VPD (Pa)',
                  'GPP (kgC m$^{-2}$ yr$^{-1}$)','NPP (kgC m$^{-2}$ yr$^{-1}$)','NEE (kgC m$^{-2}$ yr$^{-1}$)',
                  'Transp (mm month$^{-1}$)','ET (mm month$^{-1}$)','SH (W m$^{-2}$)']
